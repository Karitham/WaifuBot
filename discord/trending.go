...
func trendingManga(data *disgord.MessageCreate, args []string) {
	var desc string

	// query the trending manga
	res, err := query.MangaTrendingSearch()
	if err != nil {
		fmt.Println(err)
	}
	...
	_, er := client.CreateMessage(
		...
		&disgord.CreateMessageParams{
			Embed: &disgord.Embed{
				Title:       "Mangas Currently Trending",
				Description: desc,
				Color:       0x0e6b0e,
				Footer: &disgord.EmbedFooter{
					IconURL: "https://anilist.co/img/icons/favicon-32x32.png",
					Text: "Trending manga list created from anilist",
				},
			},
		},
	)
	if er != nil {
		fmt.Println("There was an error sending trending manga message: ", er)
	}
}
			...
			Embed: &disgord.Embed{
				...
				Description: fmt.Sprintf(
					"This is the help for the Trending Manga functionnality\n\n"+
						"Trending manga is used to display the top 10 trending mangas from anilist. Use it like so :\n"+
						"`%strendingManga",
					conf.Prefix,
				),
				Footer: &disgord.EmbedFooter{
